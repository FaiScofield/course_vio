    ...
    for (int i = 0; i < WINDOW_SIZE; i++) {
        ...
        if (corres.size() > 20) {
            ...
            if (average_parallax * 460 > 30 && m_estimator.solveRelativeRT(corres, relative_R, relative_T)) {
                ...
                // whole structure", average_parallax * 460, l);
                return true;
            }
        }
    }
...
void Estimator::MargOldFrame()
{
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++) {
        ...
        vb << para_SpeedBias[i][0], para_SpeedBias[i][1], para_SpeedBias[i][2],
            para_SpeedBias[i][3], para_SpeedBias[i][4], para_SpeedBias[i][5], para_SpeedBias[i][6],
            para_SpeedBias[i][7], para_SpeedBias[i][8];
