...
void IMU::addIMUnoise(MotionData& data)
{
    ...
    data.imu_gyro = data.imu_gyro + gyro_sqrt_cov * noise_gyro / sqrt(param_.imu_timestep) + gyro_bias_;    // 除sqrt(δt)转离散
...
MotionData IMU::MotionModel(double t)
{
    MotionData data;
    // param
    double ellipse_x = 15;  // xy平面做椭圆运动
    double ellipse_y = 20;
    double z = 1;           // z轴做正弦运动
    double K1 = 10;         // z轴的正弦频率，是x，y的k1倍
    double K = M_PI / 10;   // 水平旋转角速度，20 * K = 2pi 由于我们采取的是时间是20s, 系数K控制yaw正好旋转一圈，运动一周

    // translation
    // twb:  body frame in world frame
    Eigen::Vector3d position(ellipse_x * cos(K*t) + 5, ellipse_y * sin(K*t) + 5,  z * sin(K1*K*t) + 5);
    Eigen::Vector3d dp(-K * ellipse_x * sin(K*t),  K * ellipse_y * cos(K*t), z*K1*K * cos(K1*K*t));              // position导数，即速度 in world frame
    double K2 = K * K;
...
//        Vw = Vw + acc_w * dt;
        ...
        /// 中值积分
        MotionData imupose1 = imudata[i-1]; // imupose1是第k帧数据
        MotionData& imupose2 = imupose;     // imupose2是第k+1帧数据
        Eigen::Quaterniond dq_mid;
        Eigen::Vector3d dtheta_half_mid =  (imupose1.imu_gyro + imupose2.imu_gyro) * dt / 4.0;
        dq_mid.w() = 1;
        ...
        Eigen::Quaterniond Qwb2(Qwb);   // 这里Qwb2是Q_wbk+1，Qwb是Q_wbk
        Qwb2 = Qwb * dq_mid;
        Qwb2.normalize();

        // aw = 0.5 * [Qwb * (acc_body - acc_bias) + gw + Qwb_k+1 * (acc_body_k+1 - acc_bias) + gw]
        Eigen::Vector3d acc_w = ((Qwb * imupose1.imu_acc + gw) + (Qwb2 * imupose2.imu_acc + gw)) / 2.0;
        Qwb = Qwb2;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        Vw = Vw + acc_w * dt;
